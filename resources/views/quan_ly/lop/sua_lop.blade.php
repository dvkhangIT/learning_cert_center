    ...
    <div class="ps-3">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0 p-0">
          ...
          <li class="breadcrumb-item active" aria-current="page">Cập nhật lớp
          </li>
        </ol>
      </nav>
    </div>
      ...
      <div class="card">
        <div class="card-body p-4">
          <form id="create-user-form"
            action="{{ route('quan-ly.lop.sua-lop', $class->ma_lop) }}"
            class="row g-3 needs-validation" method="POST">
            @csrf
            @method('PUT')
            <div class="col-md-12">
              <label for="ten_lop" class="form-label">Tên lớp</label>
              <input type="text" value="{{ old('ten_lop', $class->ten_lop) }}"
                id="ten_lop"
            ...
            <div class="col-md-12">
              ...
              @enderror"
                ...
                @foreach ($courses as $course)
                  <option {{ $course->ma_kh == $class->ma_kh ? 'selected' : '' }}
                    value="{{ $course->ma_kh }}">
