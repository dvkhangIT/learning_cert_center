...
class CnttCoBanDataTable extends DataTable
{
  use DefaultConfig;
  /**
  ...
  public function dataTable(QueryBuilder $query): EloquentDataTable
  {
    return (new EloquentDataTable($query))
      ...
      ->orderColumn('ten_chung_chi', function ($query, $direction) {
   ...
   *
   * @param \App\Models\CnttCoBan $model
   * @return \Illuminate\Database\Eloquent\Builder
   */
  public function query(KetQua $model): QueryBuilder
  {
    return $model->newQuery()
      ->with('chungChi.loaiChungChi', 'hocVien') // Giữ nguyên with() để không bị lỗi N+1
      ->leftJoin('hoc_vien', 'ket_qua.ma_hv', '=', 'hoc_vien.ma_hv') // Nối bảng hoc_vien
  ...
  {
    return $this->applyDefaultHtmlConfig($this->builder(), 'cnttcoban', true)
