...
class TiengNhatN4DataTable extends DataTable
{
  ...
  public function query(ChungChi $model): QueryBuilder
  {
    return $model->newQuery()
      ...
      ->where('chung_chi.ma_loai_cc', 3)
      ->orderBy('chung_chi.ma_cc', 'asc')
      ->select('chung_chi.*', 'ket_qua.diem_tu_vung', 'ket_qua.diem_ngu_phap_doc', 'ket_qua.diem_nghe', 'ket_qua.trang_thai', 'ket_qua.ma_kq', 'ket_qua.ngay_tao as ket_qua_ngay_tao', 'ket_qua.ngay_cap_nhat as ket_qua_ngay_cap_nhat');
  }
  ...
  public function getColumns(): array
  {
    return [
      ...
      Column::make('diem_tu_vung')->title('Từ vựng')->orderable(false)->searchable(false),
      Column::make('diem_ngu_phap_doc')->title('Ngữ pháp - Đọc')->orderable(false)->searchable(false),
