...
class TiengAnhCtutDataTable extends DataTable
{
  ...
  public function dataTable(QueryBuilder $query): EloquentDataTable
  {
    return (new EloquentDataTable($query))
      ...
      ->addColumn('ten_hoc_vien', function ($query) {
        return $query->hocVien->hoten_hv . ' (' . $query->hocVien->ma_hv . ')';
      })
  ...
  public function query(ChungChi $model): QueryBuilder
  {
    return $model->newQuery()
      ...
      ->where('chung_chi.ma_loai_cc', 1)
      // ->orderBy('chung_chi.ma_cc', 'asc')
      ->select('chung_chi.*', 'ket_qua.diem_nghe', 'ket_qua.diem_doc', 'ket_qua.trang_thai', 'ket_qua.ma_kq', 'ket_qua.ngay_tao as ket_qua_ngay_tao', 'ket_qua.ngay_cap_nhat as ket_qua_ngay_cap_nhat');
  }
  ...
  public function getColumns(): array
  {
    return [
      ...
      Column::make('diem_nghe')->title('Điểm nghe')->orderable(false)->searchable(false),
      Column::make('diem_doc')->title('Điểm đọc')->orderable(false)->searchable(false),
